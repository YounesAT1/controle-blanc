...
@section('content')
  <form action="{{ route('stagiaire.store') }}" method="post" enctype="multipart/form-data" autocomplete="off"
    class="max-w-md mx-auto mt-8">
    @csrf
    <div class="mb-4">
      <label for="nom" class="block text-sm font-medium text-gray-600">Nom</label>
      <input type="text" id="nom" name="nom" placeholder="Nom du stagiaire" value="{{ old('nom') }}"
    ...
    <div class="mb-4">
      ...
      <input type="text" id="prenom" name="prenom" placeholder="Prenom du stagiaire" value="{{ old('prenom') }}"
        class="mt-1 p-2 block w-full border border-gray-300 rounded-md focus:outline-none focus:border-indigo-500">
      @error('prenom')
        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
      @enderror
    ...
    <div class="mb-4">
      ...
      <input type="email" id="email" name="email" placeholder="Email du stagiaire" value="{{ old('email') }}"
        class="mt-1 p-2 block w-full border border-gray-300 rounded-md focus:outline-none focus:border-indigo-500">
      @error('email')
        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
      @enderror
